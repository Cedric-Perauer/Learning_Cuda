                   ...
                   const float* const filter, const int filterWidth)
{
  ...
  const int2 thread_2D_pos = make_int2(idx_x,idx_y);
  ...
  float value_final = 0.0f;
	for (int y = -filter_half; y <= filter_half; y++){
		for (int x = -filter_half; x <= filter_half; x++){
			int image_r = static_cast<int>(thread_2D_pos.y + (y));
			clamp(image_r, numRows);

			int image_c = static_cast<int>(thread_2D_pos.x + (x));
			clamp(image_c, numCols);

			value_final += shared_filter[(y + filter_half)*filterWidth + (x+filter_half)] * static_cast<float>(inputChannel[image_r*numCols + image_c]);
		}
	}
                      ...
                      unsigned char* const blueChannel)
{
  ...
  int thread_1D_pos = idx_y * numCols + idx_x; 
                        ...
                        const int filterWidth)
{
  ...
  const dim3 blockSize(BLOCK_WIDTH,BLOCK_HEIGHT);

  
  //TODO:
  //Compute correct grid size (i.e., number of blocks per kernel launch)
  //from the image size and and block size.
  const dim3 gridSize(ceil(1.0 * numCols/blockSize.x),ceil(1.0 * numRows/blockSize.y));
